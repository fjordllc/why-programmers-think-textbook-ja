...
- Cloudflare のビルド環境には mdBook が無いので、`scripts/cloudflare-build.sh` がバージョン固定の公式リリースバイナリを取得してから `mdbook build` する。
- 出力は `book/`（`.gitignore` 済み。リポジトリには含めない。ビルドのたびに生成する）。
- mdBook のバージョンを上げるときは、ローカルと `scripts/cloudflare-build.sh` の `MDBOOK_VERSION` を合わせて更新する。
